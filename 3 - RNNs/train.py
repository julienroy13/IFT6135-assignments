...
def train_batch(net, criterion, optimizer, X, Y):
    ...
    # if nan, continue to nex batch
    if np.isnan(np.sum(y_out.data.cpu().numpy())):
      LOGGER.info("there was a FUCKIN nan")
      return 0.69, 38, True
    ...
    return loss.data[0], cost / batch_size, False
...
def train_model(model, args):
    ...
    for batch_num, x, y in model.dataloader:
        loss, cost, reload_ = train_batch(model.net, model.criterion, model.optimizer, x, y)
        losses += [loss]
        costs += [cost]
