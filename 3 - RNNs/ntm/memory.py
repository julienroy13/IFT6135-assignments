...
def _convolve(w, s, shift_forward=False):
    """Circular convolution implementation."""
    assert s.size(0) == 3
    if not shift_forward:
...
class NTMMemory(nn.Module):
    """Memory bank for NTM."""
    def __init__(self, N, M, shift_forward):
    ...
    def address(self, k, β, g, s, γ, w_prev):
        ...
        ŵ = self._shift(wg, s, self.shift_forward)
        w = self._sharpen(ŵ, γ)
    ...
    def _shift(self, wg, s, shift_forward):
        result = Variable(torch.zeros(wg.size()))
        for b in range(self.batch_size):
            result[b] = _convolve(wg[b], s[b], shift_forward)
        return result
